...
import string
import numpy as np
from utilis import get_full_data
import pickle
...
sample_data = pd.DataFrame(columns = ['genre_more','address',music_ID])
...
for n in range(1,len(bins_)):
    music_data = get_full_data(sample_data.iloc[bins_[n-1]:bins_[n]])
    ...
    file=open("./music_data_full/music_data_full"+str(i)+".bin","wb")
